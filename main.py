...
def key_points_angles(image):
    ...
    if not results.pose_key_pointss:
        return None
        
    key_pointss = results.pose_key_pointss.key_points
    ...
    try:
        l_elbow = calculate_angle(key_pointss[mp_pose.Posekey_points.LEFT_SHOULDER],
                                  key_pointss[mp_pose.Posekey_points.LEFT_ELBOW],
                                  key_pointss[mp_pose.Posekey_points.LEFT_WRIST])
        r_elbow = calculate_angle(key_pointss[mp_pose.Posekey_points.RIGHT_SHOULDER],
                                  key_pointss[mp_pose.Posekey_points.RIGHT_ELBOW],
                                  key_pointss[mp_pose.Posekey_points.RIGHT_WRIST])
...
# cleaning up PNG transparency overlay
...
stickers = {}
for key_points, files in sticker_files.items():
    ...
    if stk_path:
        ...
        if img is not None:
            stickers[key_points] = cv2.resize(img, (45, 45))
...
while start_screen_active:
    ...
    key_points = cv2.waitkey_points(30)
    if key_points == 27:
        cv2.destroyAllWindows()
        exit()
    elif info_screen_active and key_points & 0xFF == 32: 
        info_screen_active = False
...
def build_photostrip(frames, bg_color_bgr, placed_stickers):
    """ makes a classic photostrip layout using the images captured from the game """
    ...
    strip_w = photo_w + (margin_x * 2)  
    strip_h = margin_top + (4 * photo_h) + (3 * inter_gap) + margin_bottom
    ...
    for i, img in enumerate(frames):
        ...
        # cropping before resizing to prevent horizontal stretch
    ...
    for key_points, cx, cy in placed_stickers:
        stk = stickers.get(key_points)
        if stk is not None:
            sh, sw = stk.shape[:2]
...
while game_running:
    ...
    for name in base_names:
        ...
        angles = key_points_angles(resized_human)
    ...
    while pose_count <= 4:
        ...
        if not game_started:
            ...
            key_points = cv2.waitkey_points(1)
            if key_points == 27: 
                user_quit = True
                break
            elif key_points == 32: 
    ...
    # AI similarity results screen
    if len(captured_frames_raw) == 4:
        print("\n Captured! calculating score..")
        scores = []
        
        for idx, frame in enumerate(captured_frames_raw):
            current_angles = key_points_angles(frame)
            match_score = 0
        ...
        while results_active:
            ...
            # Header
            cv2.putText(res_ui, "YOUR AI SCORE", (220, 45), cv2.FONT_HERSHEY_TRIPLEX, 0.9, (255, 255, 255), 2)
            
            # 2x2 Grid coordinates
            grid_positions = [
                (60, 65, 320, 200),   # Topl eft
            ...
            # making the next button change according to mouse position
    ...
    if len(captured_frames_raw) == 4:
        ...
        def handle_studio_mouse(event, x, y, flags, param):
            ...
            # mouse hover check for save button (290, 360) -> (510, 420)
            ...
            # mouse hover check for play again btn (290, 440) -> (510, 500)
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                # sticker selection
                stk_key_pointss = ["pink", "purple", "teal", "yellow"]
                for idx_s, key_points in enumerate(stk_key_pointss):
                    sx = 290 + (idx_s * 85)
                    sy = 250
                    if sx <= x <= sx + 70 and sy <= y <= sy + 60:
                        active_sticker = key_points
                        notification_msg = f"Selected {key_points.title()} Star"
                        notification_time = time.time()
        ...
        while studio_active['active']:
            ...
            for idx_s, key_points in enumerate(stk_key_pointss):
                sx = 290 + (idx_s * 85)
                sy = 250
                is_selected = (active_sticker == key_points)
